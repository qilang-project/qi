...
# Redis 不设口令：只绑 127.0.0.1，且开发库里不会有真数据
RD_URL="redis://127.0.0.1:$RD_PORT/0"

起() {
    docker volume create qi-dev-pg-data >/dev/null
    docker volume create qi-dev-mysql-data >/dev/null
    docker volume create qi-dev-redis-data >/dev/null
    ...
    if [ -n "$(docker ps -q -f name="^${RD_NAME}$")" ]; then
        echo "· $RD_NAME 已在跑"
    elif [ -n "$(docker ps -aq -f name="^${RD_NAME}$")" ]; then
        docker start "$RD_NAME" >/dev/null && echo "· $RD_NAME 已启动"
    else
        docker run -d --name "$RD_NAME" --restart unless-stopped \
            -p "127.0.0.1:$RD_PORT:6379" \
            -v qi-dev-redis-data:/data \
...
等就绪() {
    ...
    for _ in $(seq 1 40); do
        if docker exec "$PG_NAME" pg_isready -U $USER -d $DB >/dev/null 2>&1 &&
           docker exec "$MY_NAME" mysqladmin ping -u$USER -p$PASS >/dev/null 2>&1 &&
           docker exec "$RD_NAME" redis-cli ping >/dev/null 2>&1; then
            echo " ✓"
            return 0
        fi
        printf '.'
    ...
    echo " ✗ 超时，看 docker logs $PG_NAME / $MY_NAME / $RD_NAME"
    return 1
}

状态() {
    echo
    for one in "$PG_NAME" "$MY_NAME" "$RD_NAME"; do
    ...
    echo "  QI_REDIS_URL=\"$RD_URL\""
}

停() { docker stop "$PG_NAME" "$MY_NAME" "$RD_NAME" >/dev/null 2>&1 || true; echo "已停（数据保留在命名卷里）"; }

删() {
    docker rm -f "$PG_NAME" "$MY_NAME" "$RD_NAME" >/dev/null 2>&1 || true
    docker volume rm qi-dev-pg-data qi-dev-mysql-data qi-dev-redis-data >/dev/null 2>&1 || true
    echo "容器与数据卷都已删除"
}
