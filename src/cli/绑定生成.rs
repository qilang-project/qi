...
fn 排版(
    ...
    if 窄数 > 0 {
        出.push_str(&format!(
            "// 注意 —— C 的 int 返回（本文件有 {} 个，行尾标了 `// C:int 返回`）：\n",
            窄数
        ));
        出.push_str(
            "//   qi 的 整数 是 64 位，C 的 int 只写寄存器低 32 位，负数返回码读回来会变成\n",
        );
        出.push_str("//   4294967295 这类大正数。要判负请自己转一下：\n");
        出.push_str("//       如果 (码 > 2147483647) { 码 = 码 - 4294967296; }\n");
        出.push_str("//\n");
    }
    if 跳过表.is_empty() {
        出.push_str("// 跳过清单：无。\n");
