...
impl<'ctx> 后端<'ctx> {
    ...
    fn 生成二元(
        ...
        if 用浮点 {
            ...
            //
            // **只对 i1 开火**，别的位宽不符一律留给 verifier。整数加宽到i64 走的是
            // zext（无符号扩展）—— 对 i1 正确，对一个装着负数的 i32 就是把 -1 变成
            // 4294967295，正是这套改动想消灭的那类静默错答案。
            // 目前 i32 到不了这里（`C整数` 的返回在 extern 调用点就按声明的符号性
            // sext/zext 成 i64 了，见 外部.rs 的 c_i32ret_s / c_i32ret_u），但这条
            // 不该靠「目前到不了」维持：真漏进来时要的是 verifier 当场拦下，
            // 而不是这里替它猜一个符号性。
            let (左宽, 右宽) = (li.get_type().get_bit_width(), ri.get_type().get_bit_width());
            if 左宽 != 右宽 && 左宽.min(右宽) == 1 {
                li = self.整数加宽到i64(li)?;
                ri = self.整数加宽到i64(ri)?;
            }
            let v: BasicValueEnum = match b.operator {
    ...
    /// 本编译单元里有没有**用户定义的**同名函数（不论元数、不论哪个包、
    /// 也算文件私有函数与 `外部 "库"` 声明）—— 声明.rs / 外部.rs 把每个声明过的
    /// 函数名都往 符号.函数 里落了一份，所以一张扁平表就够。
    ///
    /// 用途：让**无限定内置原语**给用户让路。并发.rs 的 生成同步内建、协程.rs 的
    /// 生成协程内建 挂着一批很短的裸名字（`完成` / `加锁` / `解锁` / `获取时间` /
    /// `取消协程` …），而它们在调用点的解析顺序排在用户函数**之前**。用户拿这些名字
    /// 定义了自己的函数，调用点就静默绑到运行时 FFI 上，返回一个句柄整数：
    ///
    ///   · 有类型标注 → 报「声明为聚合类型 结构体(N)，初值却是标量 整数」，
    ///     提示把人往「返回句柄的库函数」上引，跟真实原因（撞了内置名）毫无关系；
    ///   · 没有标注   → 直接段错误，退出码 None，没有任何源码线索。
    ///
    /// 所以只要这个名字被用户声明过（**哪怕元数对不上**），内置一律让路，走用户
    /// 函数的正常解析路径 —— 元数不符那边会报「没有接收 N 个实参的版本」，
    /// 是个能照着改的错误，而不是段错误。
    pub(super) fn 用户定义了同名函数(&self, name: &str) -> bool {
        self.符号.函数.contains_key(name)
    }
    ...
    fn 生成函数调用(
        ...
        // 用户自己定义了同名函数（如 `函数 取消协程(任务)`）就让路 —— 见 用户定义了同名函数。
        if call.module_qualifier.is_none()
            && call.type_arguments.is_empty()
            && !self.用户定义了同名函数(&call.callee)
        {
        ...
        // 表里全是 `完成` / `加锁` / `解锁` / `获取时间` 这种极易被用户占用的短名字，
        // 用户定义了同名函数就让路 —— 见 用户定义了同名函数。
        if !self.用户定义了同名函数(&call.callee) {
        ...
        let (f, sig) = match 解析结果 {
            ...
            None => {
                ...
                // 撞上无限定并发原语但实参个数不符（`等待组增加(组)` 少了第二个）。
                // 生成同步内建 已让路（否则就是 arguments[1] 越界 panic），到这儿
                // 谁都不认 —— 别报「未定义的函数」，那是在撒谎。
                if let Some((_, 参数种类, _)) = super::并发::同步内建表(&call.callee) {
                    return Err(format!(
                        "内置并发原语「{}」需要 {} 个实参，实际传了 {} 个。",
                        call.callee,
                        参数种类.len(),
                        call.arguments.len()
                    ));
