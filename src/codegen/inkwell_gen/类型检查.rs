...
/// 外部块里 32 位整数位置的符号性：C 的 `int` 与 `unsigned int`。
///
/// 只在 `外部` 块的类型注解里用得到，写法是类型名 `C整数` / `C无符号整数`。
/// 它们不是关键字 —— 以 ASCII 字母开头，词法上就是普通标识符，走
/// `TypeNode::自定义类型` 那条路，一个保留字都不占。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum C整数宽 {
    /// C `int` / `int32_t`：读回来要**符号扩展**，否则 -1 变 4294967295。
    有符号32,
    /// C `unsigned int` / `uint32_t`：零扩展。今天靠"被调方写 32 位寄存器顺带清高位"
    /// 恰好是对的，但那是未指定行为 —— 显式声明成 i32 才是有保证的。
    无符号32,
}

impl C整数宽 {
    /// 类型名 → 宽度。不是这两个名字就返回 None（照常当自定义类型解析）。
    pub fn 从类型名(名: &str) -> Option<Self> {
        match 名 {
            "C整数" => Some(Self::有符号32),
            "C无符号整数" => Some(Self::无符号32),
            _ => None,
        }
    }
}
...
pub struct 符号表 {
    ...
    /// 文件私有函数的签名：(包, 文件, 名, 元数) → 签名。
    /// 只有「同包同名同元数已被别的文件占了」的那些才进这儿，见 声明.rs。
    pub 私有签名: HashMap<(String, String, String, usize), 函数签名>,
    ...
    /// 外部函数里被标成 32 位的位置：函数名 → (每个形参的可选宽度, 返回值的可选宽度)。
    ///
    /// qi 的 `整数` 是 i64，而 C 的 `int` 只有 32 位 —— 用 i64 原型去接 `int` 返回值，
    /// 读到的是没被写过的高 32 位（`-1` 变 4294967295）。签名在 `函数` 表里仍登记成
    /// `整数`（类型推断毫不知情），只有建原型和调用点查这张表，按 i32 收发再补符号扩展。
    pub 外部c宽度: HashMap<String, (Vec<Option<C整数宽>>, Option<C整数宽>)>,
    ...
    /// 用户包别名 → 导入路径全名。`导入 Web.查询 作为 查` → 查 => "Web.查询"。
    ///
    /// 跟 标准库别名 同一个理由，只是踩的坑更贵：`变量 行 = 查.取一条(q)` 这种
    /// **不带类型注解**的模块限定调用，推断不出返回类型就落到 未知，而 语句.rs
    /// 的兜底会把未知的变量槽定成 **整数** —— 于是一个返回字符串的函数，
    /// 结果被 ptrtoint 成整数存进槽里。再把它喂给 字符串::等于 就是拿整数当
    /// 指针 CStr::from_ptr，段错误。qi-web 自己的 仓储.查找 就是这么坏的。
    pub 包别名: HashMap<String, String>,
}
...
/// 模块限定名 + 方法名 → 返回类型（用户包，非标准库）。
///
/// 限定名有三种可能，依次试，**顺序跟 表达式.rs 的 生成方法调用 一致**：
///   1. 别名的导入路径全名   `导入 Web.查询 作为 查` → "Web.查询"
///   2. 那条路径的末段       函数按包 的键是**包声明名**（"查询"），不是导入路径
///   3. 限定名本身           `导入 Web.查询;` 全路径导入，限定名就是包名
///   4. 都不是包名 → 按裸名解析（当前包优先）
///
/// 第 4 条是 `导入 助手;`（同一个包、另一个文件）的情况：限定名是**模块名**，
/// 不是包名，函数就注册在当前包下。表达式.rs 的调用解析也正是这么兜底的
/// —— 两边必须一致，否则「推断出的类型」和「真正调到的函数」会对不上。
fn 用户模块方法返回(表: &符号表, 限定名: &str, 方法名: &str) -> Option<Qi类型> {
    let 取 = |包: &str| {
        表.重载集于包(包, 方法名)
            .and_then(|v| v.first())
            .map(|s| s.返回)
    };
    let 别名指向 = 表.包别名.get(限定名).cloned();
    if let Some(全名) = &别名指向 {
        if let Some(t) = 取(全名) {
            return Some(t);
        }
        if let Some(末段) = 全名.rsplit('.').next() {
            if let Some(t) = 取(末段) {
                return Some(t);
            }
        }
    }
    取(限定名).or_else(|| 表.查函数返回(方法名))
}
...
pub fn 推断表达式类型(node: &AstNode, 表: &符号表) -> Qi类型 {
    match node {
        ...
        // `x 作为 T`：结果类型就是 T（语法上目标仅限基础类型关键字）
        AstNode::类型转换表达式(tc) => Qi类型::从注解(&tc.target_type),
        ...
        AstNode::方法调用表达式(mc) => {
            ...
            if let AstNode::标识符表达式(id) = mc.object.as_ref() {
                if 表.查变量(&id.name).is_none() {
                    ...
                    // 接收者是**用户模块**：`导入 助手;` 后的 助手.给个串()，
                    // 或 `导入 Web.查询 作为 查` 后的 查.取一条(q)。
                    //
                    // 这一段以前没有，掉到下面的 未知 —— 而 未知 的变量槽会被
                    // 语句.rs 兜底成整数，于是返回字符串的函数结果被 ptrtoint。
                    // 症状极难认：同一个函数不带模块名写就好的，带上就崩。
                    //
                    // 三种写法都要试，跟 表达式.rs 的调用解析**保持一致**
                    //（两边不一致的话，推断出的类型和真正调到的函数会对不上）。
                    if let Some(t) = 用户模块方法返回(表, &id.name, &mc.method_name) {
                        return t;
                    }
                }
            }
