    ...
    pub(super) fn 生成语句(
        ...
        match node {
            AstNode::变量声明(vd) => {
                ...
                // 构造子初值（有/无/成/败）与泛型定型表达式（泛型函数调用/泛型枚举
                // 构造/泛型结构体字面量）：先据注解期望生成，用其真实类型定槽 ——
                // 覆盖规则②（有注解）与规则④（无注解，按参数反推实例）。
                // 推断表达式类型 认不出这些表达式（返回 未知），必须先生成拿真实类型。
                let 构造初值: Option<(inkwell::values::BasicValueEnum<'ctx>, Qi类型)> =
                    match &vd.initializer {
                        Some(init)
                            if self.识别构造子调用(init).is_some()
                                || self.是泛型定型表达式(init) =>
                        {
                            Some(
                ...
                if let Some(init) = &vd.initializer {
                    ...
                    // 类型安全护栏：声明为聚合类型（数组/结构体）却拿到标量初值
                    // （整数/浮点/布尔）。若放行，下面的 协调到类型 会把标量 inttoptr
                    // 成垃圾指针，运行时解引用即段错误。拦在编译期给清晰报错。
                    // 典型：`变量 x: 数组<字符串> = 字符串::分割(...)`——分割返回的是
                    // 整数列表句柄，不是数组，应交给 列表库 读取。
                    // 只拦「注解为聚合 + 初值为标量」这一确定危险的组合，不碰
                    // 选项/结果/未来 的自动构造（走 构造初值 分支，vt 非标量）与
                    // FFI 整数句柄传参（那是形参协调，不经此处）。
                    if 注解已知.is_some() {
                        let 期望聚合 = matches!(类型, Qi类型::数组(_) | Qi类型::结构体(_));
                        let 实为标量 = matches!(vt, Qi类型::整数 | Qi类型::浮点数 | Qi类型::布尔);
                        if 期望聚合 && 实为标量 {
                            return Err(format!(
                                "变量 `{}` 类型不匹配：声明为聚合类型 {:?}，初值却是标量 {:?}。\
                                 若初值来自返回整数句柄的函数（如 字符串::分割 返回列表句柄），\
                                 请用 `整数` 接收句柄，再配合对应库（如 列表库）读取。",
                                vd.name, 类型, vt
                            ));
                        }
                    }
