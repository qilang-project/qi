...
pub(super) fn 注册表类型转qi(t: &str) -> Qi类型 {
    match t {
        ...
        // 「句柄」在 codegen 这侧跟 整数 完全一样，都是 i64 —— 它存在**只为
        // 给类型检查器看**：注册表原先用同一个「整数」既表示真整数
        // （时间.年() 的年份、字符串::查找() 的下标）又表示不透明句柄
        // （数据库.连接() 的连接号），检查器分不出来，只能一律当句柄沉默。
        // 于是 `变量 岁: 字符串 = 时间.年(t)` 这种错一路放到 codegen。
        // 见 semantic/单元检查.rs 的 注册表返回保守。
        "句柄" => Qi类型::整数,
