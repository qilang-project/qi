...
//!
//! ## 剩余边界（R4 后，留 R5）
//! - **轮询非 park-wake**：协作式 await/通道接收是「让出+重试」的协作式轮询，非真
//!   park（future 完成不主动唤醒特定协程，而靠下一轮 poll 命中）。语义正确、单线程
//!   下无损；大量挂起协程时有轮询开销，park-wake 队列留 R5。
//! - **同步顶层 `<- ch` 无生产者 → 忙等**：执行器已空且通道空时不检测死锁而空转（R5）。
//! - **通道背压不挂起**：`ch <- v` 无界恒成功 / 软上限满返 1，发送端不挂起（R5）。
