...
//!
//! ## Round 4：协作式 await（协程内 await 另一协程 + 真 IO 上车）
//! 协程体内 `等待 <future>` 不再走 eager 嵌套驱动，改**协作式轮询**（见 异步.rs
//! 协程等待轮询 + coro.rs `qi_coro_await_poll`）：poll 未就绪 → `qi_coro_yield_ready`
//! +挂起（控制权回执行器）；resume 再 poll；就绪后走既有取值（此刻立即返回、协程
//! future 也不再 re-entrant 驱动）。一个循环同时覆盖两类 future：
//! - **协程内 await 另一协程**：poll 看被等协程 `done`；未完成则让出，执行器驱动
//!   被等协程（甚至多层嵌套 主→子→孙）到完成再唤醒。R3 的 re-entrant 崩溃消除。
//!   铁证：示例/高级/协程/嵌套等待测（两层嵌套返回 17，RC 0）。
//! - **真 IO 上车**：poll 看 eager `Future` 的 state（`异步询问`/`异步对话` 的 HTTP
//!   pending future）；在飞则让出 → 同执行器上别的协程照跑 → future 完成（后台线程）
//!   后原协程被唤醒取结构化结果。铁证：示例/高级/协程/真IO上车测（问询员 await 真 API
//!   期间，计算员 tick 0-5 跑完，IO 回来问询员拿到「北京(明朝)」，RC 0）。
//!
//! ## Round 5：通道 park-wake（真挂起唤醒 + 背压 + 死锁检测）
//! 通道从 R3/R4 的「协作式 spin-yield」升级为**真 park-wake**：
//! - **含挂起点判定扩展**：`含等待` 现也认 `<- ch` / `ch <- v`（通道收发是挂起点）——
//!   含通道操作的函数即协程化，否则 `启动` 误走 tokio 池与协程原生通道冲突（本轮根因）。
//! - **park-wake**（coro.rs）：收空通道 → 协程摘出就绪队列、挂进 `recv_waiters`（`park_recv`）；
//!   发满有界通道 → 连同值挂进 `send_waiters`（`park_send`）。对端 try_send/try_recv 成功时
//!   `wake` 一个等待者移回 PENDING。免 R3/R4 的空转轮询。
//! - **延迟 park**（本轮关键修复，R5 第一版回退的根因）：顶层 `启动` 跑 ramp 时 CURRENT=null，
//!   协程首次 park 还没 QiCoro 对象 → park_recv/send 记 `PARK_INTENT`，`qi_coro_spawn` 建出
//!   对象后据此挂到通道等待者列表（而非 PENDING）。
//! - **背压**：有界通道满 → 发送方 park，接收方腾空位时补值+唤醒（真流控）。
//! - **死锁检测**：`执行器运行全部` 结束时 `PARKED_COUNT>0`（协程挂通道无人唤醒）→ 告警不 hang。
//! - 铁证：通道背压测（cap2 发送方被节流到消费者节奏）、死锁检测测（报错不 hang）；
//!   另 100 消费者阻塞单通道全唤醒和=4950、3 消费者 cap1 全收到，均 RC 0。吞吐不退（9M 切换/s）。
//!
//! ## 剩余边界（R5 后）
//! - `启动 <非协程表达式>` 仍走 tokio 池，与协程原生通道不互通（跨世界，设计取舍）。
//! - `异步对话`→`未来<字符串>` 的直接 `等待` 取值是既有 await 类型问题（qi_future_await_ptr
//!   不匹配 String 变体，返 null）—— 与协程无关；结构化用 `异步询问::<T>`（已验）。
...
impl<'ctx> 后端<'ctx> {
    ...
    fn 填协程cleanup(
        ...
        // （与普通函数「关 ARC 不释放」一致）。弧隐藏引用计数槽：内层块遮蔽变量
    ...
    ///
    /// 同 并发.rs 生成同步内建：这些也是无限定裸名，调用点先过
    /// 用户定义了同名函数 —— 用户声明过 `取消协程` / `协程单步` 之类的同名函数就不进来。
    pub(super) fn 生成协程内建(
        &mut self,
        callee: &str,
        arguments: &[AstNode],
...
/// 递归判定语句序列是否含**挂起点**：`等待` 或 通道收发（`<- ch` / `ch <- v`）。
/// 通道收发是协作式挂起点（收空/发满时让出+park）—— 含它们的函数必须协程化，
/// 否则 `启动` 会误走 tokio 池、与协程原生通道（单线程）冲突（R5 修）。
/// 不下钻闭包边界（闭包体的挂起点归内部）。
...
fn 节点含等待(n: &AstNode) -> bool {
    if matches!(
        n,
        AstNode::等待表达式(_) | AstNode::通道接收表达式(_) | AstNode::通道发送表达式(_)
    ) {
        return true;
    }
    if matches!(n, AstNode::闭包表达式(_)) {
        return false; // 闭包边界：内部挂起归内部（不协程化闭包）
    }
