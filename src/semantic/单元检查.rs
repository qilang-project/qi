//! 全编译单元·宽容语义类型检查器
//!
//! # 档位：默认开、默认致命（2026-08-22 起）
//!
//! `QI_TYPECHECK` 未设 = strict（报错即中止编译）；`warn`/`1` = 只打印；
//! `off` = 只留硬错。接线见 lib.rs `collect_programs_带检查`。
//!
//! 在此之前这里默认**完全不跑**，理由是「仓库里还有一批历史写法过不了」。
//! 那批在 2026-07/08 两轮补全里清完了，抬档前重新量过宽语料（qi/示例 211 个
//! 入口 + 各包与项目 221 个入口）：假阳性 0，唯一的 4 条报错是 项目/学外语/
//! 出一课.qi 里的真错。
//!
//! 关着的代价是实测的，不是推测：`变量 歪: 字符串 = "x"` 传给 整数 形参、
//! 或经变量返回错类型 —— 检查器一直抓得住（红码 06/07 常绿），而默认不跑
//! 就一路放到 codegen，跑出来是**退出码 0、打印字符串的堆地址**（`4359364632`
//! 这种），或者直接段错误。写好了却关着，等于没写。
//!
...
//!     载荷沉默）；常量重赋值查（重入声明不误伤）；数组混族查（2026-08-22
//!     门槛从「直接字面量」放宽到「字面量来源」，堵掉变量中转那条缝）。
...
fn 注册表返回保守(t: &str) -> 推断 {
    match t {
        ...
        // ── 整数返回不再一律沉默（2026-08-22）──────────────────────
        //
        // 注册表原先用同一个「整数」表示两种完全不同的东西：
        //   · 真整数 —— 时间.年() 的年份、字符串::查找() 的下标、长度、计数、0/1 判定
        //   · 不透明句柄 —— 数据库.连接()、列表.创建整数列表()
        // 检查器分不出来，只能按最保守的那种（句柄）处理，于是**全部沉默**。
        // 代价：307 条整数返回里 90 条是货真价实的整数，
        // `变量 岁: 字符串 = 时间.年(t)` 这种错一路放到 codegen。
        //
        // 现在句柄在注册表里显式写成 "句柄"（codegen 那侧跟 整数 同为 i64，
        // 零行为变化，见 inkwell_gen/导入.rs），剩下的 整数 就是真整数。
        //
        // 本轮只把**整数返回全是真值、一个句柄都没有**的模块留成 整数
        // （时间/字符串/测试/路径/反射/环境/信号/输入输出/配置/随机/正则/进程）。
        // 列表、哈希表、字节切片、gRPC、Redis、数据库 这些句柄和真值混在一起，
        // 一律先标 句柄 = 保持原样，要逐条核过才能放开。
        "整数" | "i64" | "i32" => Some(TypeNode::基础类型(BasicType::整数)),
        _ => None, // 句柄 / 数组 / 未来<..> / 其他 → 未知
    }
}
...
impl 单元检查器 {
    ...
    fn 是注册表具体返回(&self, 限定名: &str, 函数名: &str) -> bool {
        ...
        // 整数也算可信来源了 —— 前提是注册表把句柄显式标成了 "句柄"
        // （注册表返回保守 会把它映射成未知，走不到这儿）。
        //
        // 这一条原来把 整数 排除在外，理由是「整数返回是句柄语义，存进什么
        // 变量都是惯用法」。现在句柄能自报家门，那条理由就不成立了：
        // 时间.年() 返回的是年份，`变量 岁: 字符串 = 时间.年(t)` 必错。
        matches!(
            注册表返回保守(&返回),
            Some(TypeNode::基础类型(
                BasicType::字符串 | BasicType::浮点数 | BasicType::布尔 | BasicType::整数
            ))
        )
    }
