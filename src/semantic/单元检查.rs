...
fn 注册表返回保守(t: &str) -> 推断 {
    match t {
        ...
        // ── 整数返回不再沉默（2026-08-22）──────────────────────────
        //
        // 这里原先是 `_ => None`：**724 条注册表函数里，凡是返回整数的一律当
        // 未知**。理由写的是「整数返回是句柄语义（数据库.连接() 的连接号），
        // 存进什么变量都是惯用法」。代价是 300 多条真整数跟着一起沉默 ——
        // 时间.年() 的年份、字符串::查找() 的下标、各种长度和计数，
        // `变量 岁: 字符串 = 时间.年(t)` 一路放到 codegen。
        //
        // 中间做过一版「注册表里把句柄显式标成 "句柄"，只放开真整数」，
        // 然后拿全语料做了对照实验：**368 条句柄全部改成受检，567 个文件
        // 依然 0 报错**。原因很直白 —— 句柄惯用法本来就是「句柄存进 整数
        // 变量」，整数对整数根本不冲突，那条顾虑在返回类型这一侧不存在。
        //
        // 于是把 "句柄" 删了：两个行为完全一样的类型名是个陷阱，后来人
        // 一定会以为它有用。真要禁「对句柄做算术」是另一回事，那得先有
        // 二元操作数检查（目前整个是沉默的，见本文件沉默清单第 10 条）。
        "整数" | "i64" | "i32" => Some(TypeNode::基础类型(BasicType::整数)),
        _ => None, // 数组 / 未来<..> / 指针 / 其他 → 未知
    }
}
...
impl 单元检查器 {
    ...
    fn 是注册表具体返回(&self, 限定名: &str, 函数名: &str) -> bool {
        ...
        // 整数也算可信来源。这一条原来把整数排除在外，理由跟 注册表返回保守
        // 那儿一样（「整数返回是句柄语义」），而全语料实验证明那条顾虑在
        // 返回类型这一侧不成立 —— 句柄存进 整数 变量，整数对整数不冲突。
