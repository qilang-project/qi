...
impl 单元检查器 {
    ...
    /// 数组混族元素检查：`[1, "a"]` 报；任一元素不是**字面量来源**
    /// （调用返回/句柄/未知表达式…）→ 整体沉默。整数/浮点/布尔同为数值族不报。
    ///
    /// 门槛用 `是字面量来源` 而不是「直接字面量表达式本体」：后者被
    /// `变量 歪: 字符串 = "x"; 变量 表: 数组<整数> = [甲, 歪];` 一句话绕开，
    /// 而 codegen 对此毫无察觉 —— 实测跑出来是**退出码 0，打印字符串的堆地址**
    /// （`4359364632`），静默错答案里最难发现的一种。
    ///
    /// 放宽是安全的：`是字面量来源` 认的是「由字面量初始化且未被非字面量覆写
    /// 的局部变量」，它的类型跟字面量一样确定，句柄惯用法（函数返回值存进
    /// 整数变量再传）仍然落在沉默面里。
    pub(super) fn 检查纯字面量数组(
        ...
        // 门槛：所有元素都是字面量来源（字面量本体、字面量变量、字面量组合式）
        if !al.elements.iter().all(|e| self.是字面量来源(e)) {
            return;
        }
