...
impl SemanticAnalyzer {
    ...
    pub fn analyze(&mut self, ast: &crate::parser::ast::AstNode) -> Result<(), SemanticError> {
        ...
        if !self.type_checker.symbol_table.get_errors().is_empty() {
            return Err(SemanticError::ScopeError(format!(
                "作用域错误: {:?}",
                self.type_checker.symbol_table.get_errors()
            )));
        }
    ...
    fn analyze_node(&mut self, node: &crate::parser::ast::AstNode) -> Result<(), SemanticError> {
        match node {
            ...
            AstNode::结构体实例化表达式(struct_literal) => {
                self.analyze_struct_literal(struct_literal)
            }
            ...
            AstNode::数组字面量表达式(array_literal) => {
                self.analyze_array_literal(array_literal)
            }
            AstNode::字符串连接表达式(string_concat) => {
                self.analyze_string_concat(string_concat)
            }
    ...
    fn analyze_for_statement(
        ...
        let element_type = match &range_type {
            crate::parser::ast::TypeNode::数组类型(array_type) => {
    ...
    fn analyze_function_call(
        ...
        let symbol = match self
            .type_checker
            .symbol_table
            .lookup_symbol(&call_expr.callee)
        {
            Some(symbol) => symbol.clone(),
            None => {
                return Err(SemanticError::FunctionCallError(format!(
                    "未定义的函数 '{}'",
                    call_expr.callee
                )));
            }
    ...
    fn is_boolean_type(&self, type_node: &crate::parser::ast::TypeNode) -> bool {
        matches!(
            type_node,
            crate::parser::ast::TypeNode::基础类型(crate::parser::ast::BasicType::布尔)
        )
    }
    ...
    fn analyze_array_literal(
        ...
        let first_type = self
            .type_checker
            .check(&array_literal.elements[0])
            .map_err(|e| SemanticError::TypeMismatch(e.to_string(), "".to_string()))?;
    ...
    fn analyze_string_concat(
        ...
        let left_type = self
            .type_checker
            .check(&string_concat.left)
            .map_err(|e| SemanticError::TypeMismatch(e.to_string(), "".to_string()))?;

        // Check right expression
        let right_type = self
            .type_checker
            .check(&string_concat.right)
            .map_err(|e| SemanticError::TypeMismatch(e.to_string(), "".to_string()))?;
    ...
    fn is_integer_type(&self, type_node: &crate::parser::ast::TypeNode) -> bool {
        matches!(
            type_node,
            crate::parser::ast::TypeNode::基础类型(crate::parser::ast::BasicType::整数)
        )
    }
    ...
    fn is_string_type(&self, type_node: &crate::parser::ast::TypeNode) -> bool {
        matches!(
            type_node,
            crate::parser::ast::TypeNode::基础类型(crate::parser::ast::BasicType::字符串)
        )
    }
    ...
    fn analyze_method_declaration(
        ...
        self.method_system
            .register_method(receiver_type_name, method_decl.clone())
            .map_err(|e| SemanticError::ScopeError(e.to_string()))?;
    ...
    fn analyze_struct_literal(
        ...
        let symbol = match self
            .type_checker
            .symbol_table
            .lookup_symbol(&struct_literal.struct_name)
        {
            Some(symbol) => symbol.clone(),
            None => {
                return Err(SemanticError::TypeMismatch(
    ...
    fn report_type_mismatch_error(
        ...
        self.diagnostics
            .type_mismatch_error(span, expected, found, Some(&suggestion));
    }

    /// Report function call error with detailed suggestions
    /// 报告函数调用错误及详细建议
    #[allow(dead_code)]
    fn report_function_call_error(
        &mut self,
        func_name: &str,
        error_type: &str,
        span: Span,
        ...
        self.diagnostics
            .function_call_error(span, &message, Some(&suggestion));
    }
    ...
    fn report_array_access_error(&mut self, error_type: &str, span: Span, details: &str) {
        ...
        self.diagnostics
            .array_access_error(span, &message, Some(&suggestion));
    }

    /// Report struct field error with detailed suggestions
    /// 报告结构体字段错误及详细建议
    #[allow(dead_code)]
    fn report_struct_field_error(
        &mut self,
        struct_name: &str,
        field_name: &str,
        span: Span,
        ...
        self.diagnostics
            .struct_field_error(span, struct_name, field_name, Some(&suggestion));
    }
    ...
    fn report_invalid_operation_error(&mut self, operation: &str, type_name: &str, span: Span) {
        ...
        self.diagnostics
            .invalid_operation_error(span, operation, type_name, Some(&suggestion));
    }

    /// Report variable redeclaration error
    /// 报告变量重复声明错误
    #[allow(dead_code)]
    fn report_variable_redeclaration_error(
        &mut self,
        var_name: &str,
        span: Span,
        original_span: Span,
        ...
        self.diagnostics
            .variable_redeclaration_error(span, var_name, original_span);
    }
    ...
    fn report_constant_reassignment_error(&mut self, const_name: &str, span: Span) {
        self.diagnostics
            .constant_reassignment_error(span, const_name);
    }

    /// Report non-boolean condition error
    /// 报告非布尔条件错误
    #[allow(dead_code)]
    fn report_non_boolean_condition_error(
        &mut self,
        condition_type: &str,
        span: Span,
        context: &str,
        ...
        self.diagnostics
            .non_boolean_condition_error(span, condition_type, context_str);
    }
