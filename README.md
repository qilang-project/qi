...
🌐 官网：https://qilang.org/ ｜ 📦 包中心：https://pkg.qilang.org/
...
- ✅ **轻量协程** - 类似 Go 的并发模型，一个线程池跑成千上万个协程
- ✅ **通道通信** - 支持带缓冲和无缓冲通道
...
- 📖 **[语言完整指南](docs/Qi语言完整指南.md)** - 完整参考文档
- 📐 **[基础语法规范](docs/基础语法规范.md)** - 当前推荐写法与兼容边界
- 📚 **[示例程序](示例/)** - 运行示例统一从这里进入
- 🔧 **[开发指南](CLAUDE.md)** - 技术栈和开发指导
- 🐧 **[Linux 使用指南](LINUX.md)** - Linux 平台特定说明和故障排除
...
### 标准库网络示例 (`示例/标准库/网络/` 与 `示例/标准库/HTTP/`)
- **HTTP客户端示例** - HTTP 请求
- **HTTP服务器示例** - HTTP 服务端
- **TCP/UDP 示例** - 网络编程基础
...
Qi 语言使用 100% 中文关键字。全部保留字如下（比较与四则运算一律用符号 `+ - * / % == != > < >= <=`）：

| 分类     | 关键字                                                       |
| -------- | ------------------------------------------------------------ |
| 程序结构 | 包、模块、导入、导出、作为、公开、外部                       |
| 声明     | 函数、变量、常量、类型、结构体、枚举、闭包、新建、实现、特性、弱、异步 |
| 控制流   | 如果、否则、当、对于、在、到、直到、返回、跳出、继续、匹配、情况、默认 |
| 逻辑运算 | 与、且、或、非                                               |
| 布尔值   | 真、假                                                       |
| 基础类型 | 整数、浮点数、字符串、布尔、字符、字节、空、指针             |
| 复合类型 | 数组、列表、通道、未来、选项、结果                           |
| 并发     | 启动、选择、等待、超时                                       |
| 错误处理 | 尝试、捕获、抛出、最终                                       |
