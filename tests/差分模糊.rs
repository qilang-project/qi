...
impl 求值器<'_> {
    ...
    fn 算布式(
        ...
        Ok(match b {
            ...
            // ⚠ **两侧都要求值** —— qi 的 与/或 目前不短路。
            //
            // 这不是这里图省事，是实测出来的语言现状（本差分器跑出的第一个
            // 真发现，种子 777024）：
            //
            //   函数 有副作用() : 整数 { 打印行(999); 返回 1; }
            //   如果 (1 < 2 或 有副作用() > 0) { … }   // 999 照样打出来
            //   如果 (2 < 1 与 有副作用() > 0) { … }   // 999 照样打出来
            //
            // 代价不只是多跑几次：教科书守卫直接崩 ——
            //   如果 (d != 0 与 (100 / d) > 5) { … }   // d=0 时除零，退出码 None
            //
            // 求值器先跟现状对齐，免得每一批都被同一条淹掉。**如果哪天 与/或
            // 改成短路，这两行要同步改回 && / ||，否则这个差分测试会全红**。
            布表达式::与(左, 右) => {
                let a = self.算布式(左, 环境)?;
                let b = self.算布式(右, 环境)?;
                a && b
