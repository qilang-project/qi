...
/// 路径写歪就是写歪 —— **哪怕同名包正躺在某级祖先目录下也不许兜底**。
///
/// 这条原来是反着断言的（「祖先扫描仍兜底，免得历史工程升级即炸」），
/// 但 qi 2.0 那轮把祖先目录扫描**故意删掉了**：它按包名匹配、逐级翻遍每一个子目录，
/// 祖先路径上任何一份残留副本都会**静默**盖掉显式声明的依赖 ——「改了库却没生效」
/// 那类问题十有八九是它。删掉之后这条就一直红着，只是 CI 只跑
/// `cargo test --release --lib`、够不到 tests/ 下的集成测试，所以没人看见。
/// 现在按新语义反过来断言，把那个决定钉住。
#[test]
fn 路径写歪时不许靠祖先目录扫描兜底() {
    let temp = TempDir::new().unwrap();
    ...
    let 结果 = QiCompiler::new().collect_programs(&entry);
    assert!(
        结果.is_err(),
        "[依赖] 指向不存在的路径就该失败；祖先目录里有同名包也不算数"
    );
}
